 ...
 *
 * Run with: npx ts-node scripts/migrate-to-yaml.ts
 */
...
const STAGES = ['01-ideas', '02-drafts', '03-published'] as const;
...
function extractCoreInsightFromContent(content: string): string | null {
  const match = content.match(/## Core Insight\s*\n+([^\n#]+)/);
  return match ? match[1].trim() : null;
}

function cleanContent(content: string, title: string | null): string {
  let cleaned = content;

  // Remove title heading (# Title)
  if (title) {
    cleaned = cleaned.replace(new RegExp(`^#\\s+${escapeRegex(title)}\\s*\\n+`, 'm'), '');
  } else {
    // Remove first # heading regardless
...
async function migrateFile(sourcePath: string, stage: string): Promise<MigrationResult> {
  ...
  try {
    ...
    const slug = metadata.slug && metadata.slug.trim() && !metadata.slug.includes(' ')
      ? metadata.slug
...
async function migrate(): Promise<void> {
  ...
  if (failed === 0 && successful > 0) {
    ...
    console.log('3. Update content-studio to use the new structure');
  }
}

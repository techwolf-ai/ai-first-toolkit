#!/usr/bin/env bash
# Lane-discipline negative control for scope-review. The spec is SCOPE-CLEAN —
# every Won't-have has a concrete Reason, MoSCoW buckets are consistent, and §3
# lists endpoint/export NAMES only (no schema, no request/response tables, no
# DDL). It DOES carry two writing-clarity issues — an AI-flavoured filler
# sentence and an unexplained internal codename — but those belong to
# product-review's lane, not scope. A scope-review that flags them is straying
# out of lane (the regression guard for the found lane-leak). Pure-local;
# scope-review only reads the file.
set -eu
SANDBOX="$1"
...
handle time and introducing typos. In today's fast-paced support landscape, this
feature is a game-changer that empowers agents to deliver seamless, delightful
experiences at scale. This ships as part of Project Cormorant.
...
echo "seeded $SPEC (scope-clean spec with writing-clarity issues in product-review's lane)"
